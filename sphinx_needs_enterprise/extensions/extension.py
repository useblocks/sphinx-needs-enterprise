...
from sphinx_needs_enterprise.exceptions import InvalidConfigException, CommunicationException, LicenseException
...
class ServiceExtension(BaseService):
    ...
    def set_license(self, product_id, product_name, product_url, docs_url, license_key=None,
                    private_feature="f1", commercial_feature="f2", suppress_private_message=False):
        ...
        self.license = License(self.product_id, self.product_name, self.license_key,
                               product_url=product_url, docs_url=docs_url,
                               suppress_private_message=suppress_private_message)

    def check_license(self):
        if self.license is None:
            raise LicenseException('License not defined. User "set_license" to activate license '
                                   'support for the Sphinx-Needs extension.')
        ...
        for item in data:
            ...
            for name, selector in self.extra_data.items():
                if not (isinstance(selector, tuple) or isinstance(selector, list) or isinstance(selector, str)):
                    raise InvalidConfigException(f'Given selector for {name} of extra_data must be a list or tuple. '
            ...
            for name, selector in self.mappings.items():
                if not (isinstance(selector, tuple) or isinstance(selector, list) or isinstance(selector, str)):
                    raise InvalidConfigException(f'Given selector for {name} of mapping must be a list or tuple. '
                                                 f'Got {type(selector)} with value "{selector}"')
                if isinstance(selector, str):
                    # Set the "hard-coded" string as value
