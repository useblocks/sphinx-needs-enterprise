...
from sphinx_needs_enterprise.config import RSA_PUB_KEY, API_TOKEN, PRIVATE_USAGE_FEATURE, \
    COMMERCIAL_USAGE_FEATURE


class License:
    def __init__(self, product_id, product_name, license_key,
                 product_url=None, docs_url=None, suppress_private_message=False,
                 private_feature=PRIVATE_USAGE_FEATURE, commercial_feature=COMMERCIAL_USAGE_FEATURE,
                 rsa_key=RSA_PUB_KEY, api_token=API_TOKEN):
    ...
    def check(self, own_output=False):
        ...
        """
        license, message = Key.activate(token=self.token,
        ...
        if license is None:
           ...
           if not own_output:
                if not self.server_reachable:
                    ...
                    self.log.warning(f'Provided license "{self.license_key}" for extension {self.product_name} not valid.')
                    self.log.info(f'Remove the license from configuration to activate the private usage.'
                ...
                if self.is_private:
                    ...
                    if not self.suppress_private_message:
                        self.log.info(f'*************************************************************************\n'
                                      ...
                                      f'For technical support visit f{self.docs_url}.'
                                      f'\”\n*************************************************************************\n')
